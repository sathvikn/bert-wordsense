    ...
    for i in np.arange(0, len(df.index), 4):
        ...
        for sent_ctx in ['curr', 'prev']:
            ...
            for row in curr_prev_ctx.iterrows():
                ...
                indexed_tokens, tokenized_text = bert_client.preprocess(s1, target_token, s2 = s2)
                ...
                target_embeddings = bert_client.sum_layers(target_activations, -4)
...
def masking_embeds_preds(df):
    ...
    """
    mask_model = bert_client.initialize_masking_lm()
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    mask_results = []
    for i in range(len(df.index)):
        row = df.iloc[i]
        s1, s2, target_token = row['s1'], row['s2'], row['target']
        indexed_mask, _ = bert_client.preprocess(s1, target_token, s2 = s2, masking = True)
        mask_embed, preds = bert_client.mask_predictions(indexed_mask, mask_model, tokenizer)
...
def generate_sense_predictions(df, mask_results, both_sense_types, one_sense_types, available_sense):
    """
    Input:
    df- experimental stimuli
    mask_results- list of dictionaries with mask embeddings and predictions (we only use embeddings)
    both_sense_types- list of targets in stimuli where SEMCOR has data on both senses
    one_sense_types - list of targets in stimuli where SEMCOR has data on one sense
    available_sense is equal to the length of one_sense_types and gives the sense that is in SEMCOR for the corresponding type
    ...
    for w in reg_cases['target'].unique():
        ...
        if w == 'herbs':
            ...
            semcor_word = w
        token_data = bert_client.load_data(semcor_word, 'n', 'semcor')
        ...
        s1_centroid, s2_centroid = metrics.centroid(s1_embeds), metrics.centroid(s2_embeds)
    ...
    for p in homophones:
        t1, t2 = bert_client.load_data(p[0], 'n', 'semcor'), bert_client.load_data(p[1], 'n', 'semcor')
    ...
    for w, s in zip(one_sense_types, available_sense):
        ...
        if w == 'cards':
            ...
            semcor_word = w
        token_data = bert_client.load_data(w, 'n', 'semcor')
        semcor_embeddings = np.array(token_data['embeddings'])
        num_embeddings = len(semcor_embeddings)
        ...
        if w == 'glasses': #Other sense is plural form of glass.n
            glass_embeds = bert_client.load_data('glass', 'n', 'semcor')
            ...
            s1_centroid, s2_centroid = metrics.centroid(sense_embeds), metrics.centroid(item_embeds)
            ...
            one_sense_centroid += one_sense_preds(mask_embeddings, semcor_embeddings, s, other, 'centroid')
...
def check_semcor_data(word_pos_pairs, df):
    ...
    for w in word_pos_pairs.iterrows():
        ...
        if word == 'cards':
            ...
            semcor_word = word
        try:
            type_data = bert_client.load_data(semcor_word, pos, 'semcor')
            ...
            common_senses = set(word_senses).intersection(set(type_data['sense_labels'])) #set w/1 item
            ...
            pass
    return both_sense_types, one_sense_types, available_sense
...
def nearest_neighbor(query, s1, s2, sense_names):
    ...
    """
    s1_sims = [metrics.cosine_sim(query, e) for e in s1]
    s2_sims = [metrics.cosine_sim(query, e) for e in s2]
...
def centroid_pred(query, c1, c2, sense_names):
    ...
    c1_sim = metrics.cosine_sim(query, c1)
    c2_sim = metrics.cosine_sim(query, c2)
    if c1_sim > c2_sim:
        return sense_names[0]
    if c1_sim < c2_sim:
        return sense_names[1]
...
def one_sense_preds(query_embeds, sense_embeds, data_sense, other_sense, method):
    ...
    """
    if method == "nn":
        cos_sims = [max([metrics.cosine_sim(q, s) for s in sense_embeds]) for q in query_embeds]
    if method == 'centroid':
        c = metrics.centroid(sense_embeds)
        cos_sims = [metrics.cosine_sim(q, c) for q in query_embeds]
...
def accuracy_table(all_data, predicate):
    ...
    return pd.DataFrame({predicate: all_data[predicate].unique(),
                         "centroid_acc": [accuracy_by_predicate(all_data, predicate,
                                                                 p, 'centroid_preds') for p in all_data[predicate].unique()],
             "nn_acc": [accuracy_by_predicate(all_data, predicate, p, 'nn_preds') for p in all_data[predicate].unique()]})
