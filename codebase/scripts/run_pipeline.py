...
def configure_parser():
    parser = argparse.ArgumentParser("Arguments to the pipeline")
    parser.add_argument("--type", help = "For individual types (word.pos)")
    parser.add_argument("--file", help = "Path to a file with words(CSV with one row per type and columns for word and pos)")
    parser.add_argument("--output_dir", help = "Location of directory where results will be stored", default="semcor")
    parser.add_argument("--min_instances", help = "Minimum number of instances for each sense", default = 10)
    parser.add_argument("--plotting", help = "Store graphs", action = 'store_true')
